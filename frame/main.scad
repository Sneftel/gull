...
module SowSide()
{
    pn_attach("front") SowPelvis_Anchored() rotate([90,0,0])
    color("#5B64C9")
    translate([0,0,-THICKNESS/2]) linear_extrude(ACTUAL_THICKNESS) 
        pn_top() Spine_Anchored();

    pn_attach("rear") SowPelvis_Anchored() rotate([90,0,0]) {
        color("#3B44A9") translate([0,0,-THICKNESS/2]) linear_extrude(ACTUAL_THICKNESS) 
            pn_top() Spine_Anchored();
...
module SowMain()
{
    translate([0,0,SPINE_BASE_HEIGHT-THICKNESS/2])
    {
        translate([0,0,-THICKNESS/2]) linear_extrude(ACTUAL_THICKNESS) pn_top() SowPelvis_Anchored();
